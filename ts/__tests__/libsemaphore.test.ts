...
const provingKey: libsemaphore.SnarkProvingKey = fs.readFileSync(
  provingKeyPath
);
const verifyingKey: libsemaphore.SnarkVerifyingKey = libsemaphore.parseVerifyingKeyJson(
...
describe("libsemaphore", function () {
  ...
  it("serialiseIdentity() and unSerialiseIdentity() should work", async () => {
    ...
    const unSerialisedId: libsemaphore.Identity = libsemaphore.unSerialiseIdentity(
      serialisedId
    );

    expect(unSerialisedId).toEqual(identity);
    expect(unSerialisedId.identityNullifier).toEqual(
      identity.identityNullifier
    );
    expect(unSerialisedId.identityTrapdoor).toEqual(identity.identityTrapdoor);
    expect(unSerialisedId.keypair.privKey.toString("hex")).toEqual(
  ...
  it("identityCommitment() should produce a value of the correct length and type", async () => {
    const idc: libsemaphore.SnarkBigInt = libsemaphore.genIdentityCommitment(
      identity
    );
  ...
  it("genMixerSignal should return a hash", async () => {
    const signal = libsemaphore.genMixerSignal("0xabcd", "0xdefd", 0);
  ...
  it("genProof() should generate a valid proof", async () => {
    ...
    const isValid = libsemaphore.verifyProof(
      verifyingKey,
      proof,
      publicSignals
    );
  ...
  it("genBroadcastSignalParams() should produce the correct params", async () => {
    const params = libsemaphore.genBroadcastSignalParams(
      witnessData,
      proof,
      publicSignals
    );
  ...
  test("genExternalNullifier() should always return a 32-byte hex string whose true size is 29 bytes", () => {
    ...
    expect(fullHash).toEqual(
      "0x51480a3453be7db7a786adbfc5d579a36a620c26f5a2e51d4c296d52892e38d6"
    );
    expect(hash).toEqual(
      "0x0000003453be7db7a786adbfc5d579a36a620c26f5a2e51d4c296d52892e38d6"
    );
